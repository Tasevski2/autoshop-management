    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" render={<Link to={`/customers/${id}/edit`} />}>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Button size="sm" render={<Link to={`/vehicles/new?customer=${id}`} />}>
            <Plus className="mr-1 h-3.5 w-3.5" />
            {t('customers.addVehicle')}
          </Button>
        </div>
        {vehicles.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t('customers.noVehicles')}</p>
        ) : (
                ...
                <Link key={v.id} to={`/vehicles/${v.id}`}>
                  <Card className="hover:bg-accent/50 transition-colors cursor-pointer">
                    <CardContent className="pt-4 pb-4">
                      <p className="font-mono font-bold text-base">{v.plate_number}</p>
          ...
          <>
            <div className="rounded-md border overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {services.map((s) => {
                    const vehicle = s.vehicles as { plate_number: string; brand: string; model: string } | null
                    return (
